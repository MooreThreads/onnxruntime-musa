...
MusaEpFactory::MusaEpFactory(const OrtApi& ort_api, const OrtEpApi& ep_api,
  ...
  data_transfer_impl_ = std::make_unique<MusaDataTransfer>(ort_api, ep_api, device);
  ...
  readonly_memory_info_ = Ort::MemoryInfo{"MUSAExecutionProvider CPU readonly",
...
OrtStatus* MusaEpFactory::GetKernelRegistryForEp(MusaEp& ep,
...
/*static*/
const char* ORT_API_CALL MusaEpFactory::GetNameImpl(const OrtEpFactory* this_ptr) noexcept {
  const auto* factory = static_cast<const MusaEpFactory*>(this_ptr);
  return factory->ep_name_.c_str();
}

/*static*/
const char* ORT_API_CALL MusaEpFactory::GetVendorImpl(const OrtEpFactory* this_ptr) noexcept {
  const auto* factory = static_cast<const MusaEpFactory*>(this_ptr);
  return factory->vendor_.c_str();
}

/*static*/
uint32_t ORT_API_CALL MusaEpFactory::GetVendorIdImpl(const OrtEpFactory* this_ptr) noexcept {
  const auto* factory = static_cast<const MusaEpFactory*>(this_ptr);
  return factory->vendor_id_;
}

/*static*/
const char* ORT_API_CALL MusaEpFactory::GetVersionImpl(const OrtEpFactory* this_ptr) noexcept {
  const auto* factory = static_cast<const MusaEpFactory*>(this_ptr);
  return factory->ep_version_.c_str();
}

/*static*/
OrtStatus* ORT_API_CALL MusaEpFactory::GetSupportedDevicesImpl(OrtEpFactory* this_ptr,
  ...
  if (musaGetDeviceCount(&musa_device_count) != musaSuccess || musa_device_count <= 0) {
    return nullptr;
  }
  ...
  for (size_t i = 0; i < num_devices && num_ep_devices < max_ep_devices; ++i) {
    ...
    if (hw_type == OrtHardwareDeviceType::OrtHardwareDeviceType_CPU) {
      ...
      factory->ort_api_.AddKeyValuePair(ep_metadata, "supported_devices", std::to_string(musa_device_count).c_str());
...
/*static*/
OrtStatus* ORT_API_CALL MusaEpFactory::CreateEpImpl(OrtEpFactory* this_ptr,
  ...
  *ep = nullptr;
  ...
  RETURN_IF_ERROR(GetSessionConfigEntryOrDefault(*session_options, "ep.musa.enable_prepack_weight_sharing",
...
/*static*/
void ORT_API_CALL MusaEpFactory::ReleaseEpImpl(OrtEpFactory* /*this_ptr*/, OrtEp* ep) noexcept {
  delete static_cast<MusaEp*>(ep);
}

/*static*/
OrtStatus* ORT_API_CALL MusaEpFactory::CreateAllocatorImpl(OrtEpFactory* this_ptr,
...
/*static*/
void ORT_API_CALL MusaEpFactory::ReleaseAllocatorImpl(OrtEpFactory* /*this_ptr*/,
                                                               OrtAllocator* allocator) noexcept {
  delete static_cast<CustomAllocator*>(allocator);
}

/*static*/
OrtStatus* ORT_API_CALL MusaEpFactory::CreateDataTransferImpl(OrtEpFactory* this_ptr,
...
/*static*/
bool ORT_API_CALL MusaEpFactory::IsStreamAwareImpl(const OrtEpFactory* /*this_ptr*/) noexcept {
  return false;
}

/*static*/
OrtStatus* ORT_API_CALL MusaEpFactory::CreateSyncStreamForDeviceImpl(OrtEpFactory* /*this_ptr*/,
