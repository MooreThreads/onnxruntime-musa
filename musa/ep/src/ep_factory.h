...
class MusaEpFactory : public OrtEpFactory {
 public:
  MusaEpFactory(const OrtApi& ort_api, const OrtEpApi& ep_api, const OrtLogger& default_logger);
  ~MusaEpFactory();
  ...
  OrtStatus* GetKernelRegistryForEp(MusaEp& ep, /*out*/ const OrtKernelRegistry** kernel_registry);
 ...
 private:
  ...
  std::unique_ptr<MusaDataTransfer> data_transfer_impl_;  // data transfer implementation for this factory
