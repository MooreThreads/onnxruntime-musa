...
// Thread-local profiling state. Tracks the active profiler ID and a per-thread
// stack of ORT event boundaries. Per-thread state is necessary because a single
// session profiler (single profiler_id) can have StartEvent/StopEvent called
// from multiple threads (e.g., via inter-op parallelism).
struct ThreadLocalProfilingState {
  std::optional<uint64_t> profiler_id;
  std::vector<size_t> ort_event_start_indices;  // Stack of event indices at
                                                // push time (per-thread)
};
...
void EpEventManager::PushOrtEvent(uint64_t profiler_id) {
  ...
  tls_profiling_state_.ort_event_start_indices.push_back(
      iter->second.events.size());

  // Set the active profiler for this thread so kernels can find it.
  tls_profiling_state_.profiler_id = profiler_id;
}

void EpEventManager::PopOrtEvent(uint64_t profiler_id,
                                 const std::string& ort_event_name,
                                 int64_t ort_event_start_us,
                                 int64_t ort_event_duration_us) {
  std::lock_guard<std::mutex> lock(mutex_);

  auto iter = profiler_state_.find(profiler_id);
  if (iter == profiler_state_.end() ||
      tls_profiling_state_.ort_event_start_indices.empty()) {
    return;
  }
  ...
  // Annotate this thread's EP events (added since StartEvent) with metadata
  // from the correlated ORT event.
  auto current_thread_id = std::this_thread::get_id();
  for (size_t i = start_index; i < iter->second.events.size(); ++i) {
    Event& ep_event = iter->second.events[i];

    if (ep_event.thread_id == current_thread_id &&
        ep_event.ort_event_name.empty()) {
  ...
  // Clear the thread-local when the outermost ORT event on this thread
  // finishes.
...
void EpEventManager::ConsumeEvents(uint64_t profiler_id,
...
MusaEpProfiler::MusaEpProfiler(const OrtEpApi& api)
...
/*static*/
void ORT_API_CALL
MusaEpProfiler::ReleaseImpl(OrtEpProfilerImpl* this_ptr) noexcept {
  delete static_cast<MusaEpProfiler*>(this_ptr);
}

/*static*/
OrtStatus* ORT_API_CALL MusaEpProfiler::StartProfilingImpl(
    OrtEpProfilerImpl* this_ptr,
    int64_t ep_profiling_start_offset_ns) noexcept {
  auto* self = static_cast<MusaEpProfiler*>(this_ptr);

  // Store the offset from ORT's profiling start (measured with ORT's clock) and
  // capture the EP's own clock. This allows computing ORT-relative timestamps
  // without depending on matching clock epochs.
  self->ep_profiling_start_offset_ns_ = ep_profiling_start_offset_ns;
  self->ep_profiling_start_time_point_ =
...
MusaEpProfiler::StartEventImpl(OrtEpProfilerImpl* this_ptr,
...
/*static*/
OrtStatus* ORT_API_CALL MusaEpProfiler::StopEventImpl(
    OrtEpProfilerImpl* this_ptr, uint64_t /*ort_event_correlation_id*/,
  ...
  // Annotate all EP events that were collected during this ORT event with
  // metadata from the ORT event.
  ep_event_manager.PopOrtEvent(self->profiler_id, ort_event_name,
                               ort_event_start_us, ort_event_duration_us);
  return nullptr;
  EXCEPTION_TO_RETURNED_STATUS_END
}

/*static*/
OrtStatus* ORT_API_CALL MusaEpProfiler::EndProfilingImpl(
    OrtEpProfilerImpl* this_ptr,
  ...
  for (EpEventManager::Event& raw_ep_event : raw_ep_events) {
    // ORT requires event timestamps (in microseconds) relative to ORT's
    // profiling start time. We compute this without depending on matching clock
    // epochs by combining:
    //   1. ep_profiling_start_offset_ns_: elapsed time (ORT clock) from ORT's
    //   profiling start to StartProfiling call.
    //   2. ep_elapsed_ns: elapsed time (EP clock) from our StartProfiling
    //   capture to this event.
    // Their sum is the event's offset from ORT's profiling start.
    int64_t ep_elapsed_ns =
        std::chrono::duration_cast<std::chrono::nanoseconds>(
            raw_ep_event.start_time - self->ep_profiling_start_time_point_)
            .count();
    int64_t rel_ts_us =
        (self->ep_profiling_start_offset_ns_ + ep_elapsed_ns) / 1000;

    int64_t dur_us = std::chrono::duration_cast<std::chrono::microseconds>(
                         raw_ep_event.end_time - raw_ep_event.start_time)
                         .count();

    // The ORT-to-EP clock reconstruction can differ by a few microseconds on
    // some platforms. Bound the EP event to the correlated ORT parent interval
    // so the emitted child event remains properly nested without weakening the
    // test's containment checks.
    if (raw_ep_event.ort_event_start_us >= 0 &&
        raw_ep_event.ort_event_duration_us >= 0) {
      const int64_t parent_start_us = raw_ep_event.ort_event_start_us;
      const int64_t parent_end_us =
          parent_start_us + raw_ep_event.ort_event_duration_us;
    ...
    // Set parent_name as an event arg. The parent_name is just the name of the
    // correlated ORT event.
    std::unordered_map<std::string, std::string> args = {
        {"parent_name", raw_ep_event.ort_event_name.c_str()}};

    Ort::ProfilingEvent event(OrtProfilingEventCategory_KERNEL, -1, -1,
                              raw_ep_event.name.c_str(), rel_ts_us, dur_us,
                              args);
