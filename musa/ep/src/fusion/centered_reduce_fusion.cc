...
Ort::ConstNode FindSecondReduceNode(Ort::ConstGraph graph) {
  for (Ort::ConstNode node : graph.GetNodes()) {
    ...
    Ort::ConstNode mul_node = ProducerNode(inputs[0]);
...
std::unique_ptr<FusionNodeCompute> CreateCenteredReduceFusion(
  ...
  Ort::ConstNode second_reduce_node = FindSecondReduceNode(graph);
  ...
  Ort::ConstNode mul_node = ProducerNode(second_reduce_inputs[0]);
  ...
  Ort::ConstNode sub_node = ProducerNode(mul_inputs[0]);
  ...
  Ort::ConstNode first_reduce_node = ProducerNode(sub_inputs[1]);
