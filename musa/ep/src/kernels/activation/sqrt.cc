...
bool TryMudnnSqrt(Ort::KernelContext& ctx, const std::vector<int64_t>& shape,
  ...
  ::musa::dnn::Unary op;
  if (op.SetMode(::musa::dnn::Unary::Mode::SQRT) !=
...
OrtStatus* Sqrt::Compute(Ort::KernelContext& ctx) const {
  ...
  if (TryMudnnSqrt(ctx, info.GetShape(), elem_type)) {
    return nullptr;
  }
  return UnaryDeviceCompute(ctx, info.GetShape(), elem_type, MusaUnaryOp::Sqrt,
                            "Sqrt");
}
}  // namespace

ONNX_OPERATOR_VERSIONED_KERNEL_EX(
    Sqrt, kOnnxDomain, 13, 19,
