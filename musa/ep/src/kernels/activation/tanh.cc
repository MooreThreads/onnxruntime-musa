...
bool TryMudnnTanh(Ort::KernelContext& ctx, const std::vector<int64_t>& shape,
  ...
  ::musa::dnn::Unary op;
  if (op.SetMode(::musa::dnn::Unary::Mode::TANH) !=
...
OrtStatus* Tanh::Compute(Ort::KernelContext& ctx) const {
  ...
  if (TryMudnnTanh(ctx, info.GetShape(), elem_type)) {
    return nullptr;
  }
  return UnaryDeviceCompute(ctx, info.GetShape(), elem_type, MusaUnaryOp::Tanh,
                            "Tanh");
}
}  // namespace

ONNX_OPERATOR_VERSIONED_KERNEL_EX(
    Tanh, kOnnxDomain, 13, 19,
