...
namespace {
template <typename T>
OrtStatus* EqualComputeTyped(Ort::KernelContext& ctx,
...
OrtStatus* Equal::Compute(Ort::KernelContext& ctx) const {
  ...
  if (elem_type == ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT)
    return EqualComputeTyped<float>(ctx, shape0, shape1);
  if (elem_type == ONNX_TENSOR_ELEMENT_DATA_TYPE_INT32)
    return EqualComputeTyped<int32_t>(ctx, shape0, shape1);
  if (elem_type == ONNX_TENSOR_ELEMENT_DATA_TYPE_INT64)
    return EqualComputeTyped<int64_t>(ctx, shape0, shape1);
  if (elem_type == ONNX_TENSOR_ELEMENT_DATA_TYPE_BOOL)
    return EqualComputeTyped<uint8_t>(ctx, shape0, shape1);
  return Ort::GetApi().CreateStatus(ORT_NOT_IMPLEMENTED,
                                    "Equal unsupported dtype");
}
}  // namespace

ONNX_OPERATOR_VERSIONED_KERNEL_EX(
    Equal, kOnnxDomain, 13, 17,
