...
namespace {
template <typename T>
OrtStatus* GreaterComputeTyped(Ort::KernelContext& ctx,
...
OrtStatus* Greater::Compute(Ort::KernelContext& ctx) const {
  ...
  if (elem_type == ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT)
    return GreaterComputeTyped<float>(ctx, shape0, shape1);
  if (elem_type == ONNX_TENSOR_ELEMENT_DATA_TYPE_INT32)
    return GreaterComputeTyped<int32_t>(ctx, shape0, shape1);
  if (elem_type == ONNX_TENSOR_ELEMENT_DATA_TYPE_INT64)
    return GreaterComputeTyped<int64_t>(ctx, shape0, shape1);
  return Ort::GetApi().CreateStatus(ORT_NOT_IMPLEMENTED,
                                    "Greater unsupported dtype");
}
}  // namespace

ONNX_OPERATOR_VERSIONED_KERNEL_EX(
    Greater, kOnnxDomain, 13, 17,
