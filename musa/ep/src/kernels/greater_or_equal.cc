...
namespace {
template <typename T>
OrtStatus* GreaterOrEqualTyped(Ort::KernelContext& ctx,
...
OrtStatus* GreaterOrEqual::Compute(Ort::KernelContext& ctx) const {
  ...
  if (elem_type == ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT)
    return GreaterOrEqualTyped<float>(ctx, shape0, shape1);
  if (elem_type == ONNX_TENSOR_ELEMENT_DATA_TYPE_INT32)
    return GreaterOrEqualTyped<int32_t>(ctx, shape0, shape1);
  if (elem_type == ONNX_TENSOR_ELEMENT_DATA_TYPE_INT64)
    return GreaterOrEqualTyped<int64_t>(ctx, shape0, shape1);
  return Ort::GetApi().CreateStatus(ORT_NOT_IMPLEMENTED,
                                    "GreaterOrEqual: unsupported dtype");
}
}  // namespace

ONNX_OPERATOR_VERSIONED_KERNEL_EX(
    GreaterOrEqual, kOnnxDomain, 12, 17,
