...
bool TryMudnnCeil(Ort::KernelContext& ctx, const std::vector<int64_t>& shape,
  ...
  ::musa::dnn::Unary op;
  if (op.SetMode(::musa::dnn::Unary::Mode::CEIL) !=
...
class Ceil : public OpKernelBase<Ceil> {
 public:
  Ceil(const OrtKernelInfo* /*info*/, void* /*state*/) {}
  OrtStatus* Compute(Ort::KernelContext& ctx) const;
};

OrtStatus* Ceil::Compute(Ort::KernelContext& ctx) const {
  auto info = ctx.GetInput(0).GetTensorTypeAndShapeInfo();
  auto elem_type = info.GetElementType();
  auto shape = info.GetShape();
  if (TryMudnnCeil(ctx, shape, elem_type)) {
    return nullptr;
  }
  return UnaryDeviceCompute(ctx, shape, elem_type, MusaUnaryOp::Ceil, "Ceil");
}
}  // namespace
