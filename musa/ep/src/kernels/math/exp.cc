...
bool TryMudnnExp(Ort::KernelContext& ctx, const std::vector<int64_t>& shape,
  ...
  ::musa::dnn::Unary op;
  if (op.SetMode(::musa::dnn::Unary::Mode::EXP) !=
...
OrtStatus* Exp::Compute(Ort::KernelContext& ctx) const {
  ...
  return UnaryDeviceCompute(ctx, shape, elem_type, MusaUnaryOp::Exp, "Exp");
}
}  // namespace

ONNX_OPERATOR_VERSIONED_KERNEL_EX(
    Exp, kOnnxDomain, 13, 19,
