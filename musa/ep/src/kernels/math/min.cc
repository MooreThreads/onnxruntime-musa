...
template <typename T>
OrtStatus* ComputeMinCpuMetadataTyped(
  ...
  if (total > kMusaMaxBroadcastRank) {
    return Ort::GetApi().CreateStatus(
        ORT_NOT_IMPLEMENTED,
        "Min CPU metadata path only supports small shape tensors");
  }
...
OrtStatus* ComputeMinCpuMetadata(
    Ort::KernelContext& ctx, const std::vector<std::vector<int64_t>>& shapes,
    const std::vector<int64_t>& output_shape,
    ONNXTensorElementDataType elem_type) {
  musaStream_t stream = GetComputeStream(ctx);
  if (elem_type == ONNX_TENSOR_ELEMENT_DATA_TYPE_INT64) {
    return ComputeMinCpuMetadataTyped<int64_t>(ctx, shapes, output_shape,
                                               stream);
  }
  if (elem_type == ONNX_TENSOR_ELEMENT_DATA_TYPE_INT32) {
    return ComputeMinCpuMetadataTyped<int32_t>(ctx, shapes, output_shape,
                                               stream);
  }
  return Ort::GetApi().CreateStatus(
      ORT_NOT_IMPLEMENTED,
      "Min CPU metadata path only supports int32/int64 tensors");
}
...
OrtStatus* ComputeDeviceMinFallback(
  ...
  if (!ToMusaElementType(elem_type, musa_elem_type) || !AllGpuInputs(ctx)) {
    if (!AllGpuInputs(ctx)) {
      return ComputeMinCpuMetadata(ctx, shapes, output_shape, elem_type);
    }
