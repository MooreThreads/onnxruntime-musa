...
OrtStatus* TryScalarInt32Min(
  ...
  return LaunchStatus(LaunchMusaBinaryScalarInt32Kernel(
      ...
      NumElements(output_shape), lhs_scalar, MusaBinaryOp::Min, nullptr));
}
...
OrtStatus* Min::Compute(Ort::KernelContext& ctx) const {
  ...
  bool handled = false;
  OrtStatus* scalar_status = TryScalarInt32Min(ctx, shapes, out_shape, elem_type,
                                               handled);
  if (handled) {
