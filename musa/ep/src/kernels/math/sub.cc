...
class Sub : public OpKernelBase<Sub> {
 public:
  Sub(const OrtKernelInfo* /*info*/, void* /*state*/) {}
  OrtStatus* Compute(Ort::KernelContext& ctx) const;
};

OrtStatus* Sub::Compute(Ort::KernelContext& ctx) const {
  ...
  if (TryMudnnSub(ctx, shape0, shape1, elem_type)) {
    return nullptr;
  }
  return BinaryDeviceCompute(ctx, shape0, shape1, elem_type, MusaBinaryOp::Sub,
                             "Sub");
}
}  // namespace

ONNX_OPERATOR_VERSIONED_KERNEL_EX(Sub, kOnnxDomain, 13, 13,
                                  (Ort::KernelDefBuilder().AddTypeConstraint(
                                      "T", BinaryNumericOpset13TensorTypes())),
                                  Sub)

ONNX_OPERATOR_VERSIONED_KERNEL_EX(Sub, kOnnxDomain, 14, 19,
                                  (Ort::KernelDefBuilder().AddTypeConstraint(
