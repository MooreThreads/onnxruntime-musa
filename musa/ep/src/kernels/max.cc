...
namespace {
template <typename T>
OrtStatus* MinMaxComputeTyped(Ort::KernelContext& ctx, bool is_max) {
  ...
  for (int64_t i = 0; i < NumElements(out_shape); ++i) {
    ...
    for (size_t input_idx = 1; input_idx < inputs.size(); ++input_idx) {
      ...
      value = is_max ? std::max(value, other) : std::min(value, other);
    }
...
OrtStatus* Max::Compute(Ort::KernelContext& ctx) const {
  ...
  if (elem_type == ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT) {
    ...
    return MinMaxComputeTyped<float>(ctx, true);
  }
  if (elem_type == ONNX_TENSOR_ELEMENT_DATA_TYPE_INT32)
    return MinMaxComputeTyped<int32_t>(ctx, true);
  if (elem_type == ONNX_TENSOR_ELEMENT_DATA_TYPE_INT64)
    return MinMaxComputeTyped<int64_t>(ctx, true);
  return Ort::GetApi().CreateStatus(ORT_NOT_IMPLEMENTED, "Max unsupported dtype");
}
}  // namespace

ONNX_OPERATOR_VERSIONED_KERNEL_EX(
    Max, kOnnxDomain, 13, 17,
    (Ort::KernelDefBuilder().AddTypeConstraint("T", AllTensorTypes())), Max)
