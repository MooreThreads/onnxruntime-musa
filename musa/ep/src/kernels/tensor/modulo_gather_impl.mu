#include "tensor/modulo_gather_impl.h"
...
__global__ void ModuloGatherKernel(const void* table, const void* indices,
                                   void* output, int32_t element_size,
                                   int32_t index_element_size,
                                   int64_t indices_count, int64_t table_rows,
                                   int64_t block_size, int64_t modulus,
                                   int64_t offset, int64_t invalid_value,
  ...
  for (int64_t output_index = thread_id; output_index < output_count;
    ...
    int64_t gather_index = raw_index == invalid_value ? 0 : remainder + offset;
...
musaError_t LaunchMusaModuloGatherKernel(
  ...
  ModuloGatherKernel<<<BlocksForCount(output_count), kThreadsPerBlock, 0,
                       stream>>>(table, indices, output, element_size,
                                 index_element_size, indices_count, table_rows,
                                 block_size, modulus, offset, invalid_value,
                                 output_count);
