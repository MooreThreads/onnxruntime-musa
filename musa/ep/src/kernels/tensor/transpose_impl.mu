...
musaError_t LaunchRank4TransposeTyped(const void* input, void* output,
  ...
  if (perm_0231) {
    ...
    const int64_t d0 = params.output_dims[0];
    const int64_t d1 = params.output_dims[3];
    const int64_t flattened_inner = params.output_dims[1] * params.output_dims[2];
    const dim3 block(kTileDim, kBlockRows, 1);
    const dim3 grid(static_cast<unsigned int>((flattened_inner + kTileDim - 1) /
                                              kTileDim),
...
musaError_t LaunchRank4Transpose0213Vector64(const void* input, void* output,
  ...
  if (inner_bytes <= 0 || inner_bytes % static_cast<int64_t>(sizeof(uint64_t)) != 0) {
    return musaErrorNotSupported;
  }
...
musaError_t LaunchMusaTransposeKernel(const void* input,
  ...
  if (params.rank == 4) {
    ...
    if (element_size == 1) {
      status = LaunchRank4TransposeTyped<uint8_t>(input, output, params, stream);
    } else if (element_size == 2) {
      status = LaunchRank4TransposeTyped<uint16_t>(input, output, params,
                                                   stream);
    } else if (element_size == 4) {
      status = LaunchRank4TransposeTyped<uint32_t>(input, output, params,
                                                   stream);
    } else if (element_size == 8) {
      status = LaunchRank4TransposeTyped<uint64_t>(input, output, params,
                                                   stream);
    }
    if (status != musaErrorNotSupported) {
      return status;
