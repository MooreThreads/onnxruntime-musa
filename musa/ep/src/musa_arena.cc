...
// for the MUSA plugin EP arena allocator, preserving the ORT CUDA plugin
// contract.
...
std::string GetAllocatorName(const OrtApi& api, OrtAllocator& allocator) {
  ...
  auto* status =
      api.MemoryInfoGetName(mem_info, &allocator_name);  // never fails
  static_cast<void>(status);
  return allocator_name;
}
}  // namespace

ArenaImpl::ArenaImpl(AllocatorUniquePtr allocator, const ArenaConfig& config,
  ...
  MUSA_ARENA_LOG(
      INFO,
      "Creating ArenaImpl for "
          << allocator_name_
          << " with following configs: initial_chunk_size_bytes: "
          << config_.initial_chunk_size_bytes
          << " max_dead_bytes_per_chunk: " << config_.max_dead_bytes_per_chunk
          << " initial_growth_chunk_size_bytes: "
          << config_.initial_growth_chunk_size_bytes
          << " max_power_of_two_extend_bytes: "
          << config_.max_power_of_two_extend_bytes
          << " memory limit: " << config_.max_mem
          << " arena_extend_strategy: " << config_.arena_extend_strategy);

  curr_region_allocation_bytes_ = RoundedBytes(std::min(
      config_.max_mem, static_cast<size_t>(config_.initial_chunk_size_bytes)));

  stats_.bytes_limit =
      config.max_mem > static_cast<size_t>(std::numeric_limits<int64_t>::max())
          ? std::numeric_limits<int64_t>::max()
          : static_cast<int64_t>(config.max_mem);

  // Create bins of various sizes.
  MUSA_ARENA_LOG(VERBOSE, "Creating " << kNumBins << " bins of max chunk size "
                                      << BinNumToSize(0) << " to "
                                      << BinNumToSize(kNumBins - 1));

  for (BinNum b = 0; b < kNumBins; b++) {
    size_t bin_size = BinNumToSize(b);
    new (BinFromIndex(b)) Bin(this, bin_size);
    MUSA_ARENA_ENFORCE((BinForSize(bin_size) == BinFromIndex(b) &&
                        BinForSize(bin_size + 255) == BinFromIndex(b) &&
                        BinForSize(bin_size * 2 - 1) == BinFromIndex(b)),
                       "Invalid bin size for bin " << b);

    if (b + 1 < kNumBins) {
      MUSA_ARENA_ENFORCE(BinForSize(bin_size * 2) != BinFromIndex(b),
                         "Invalid bin size for " << b);
    }
  }
}
...
ArenaImpl::Chunk* ArenaImpl::ChunkFromHandle(ChunkHandle h) {
  MUSA_ARENA_ENFORCE(h < chunks_.size(), "ChunkFromHandle");
  return &(chunks_[h]);
}

OrtStatus* ArenaImpl::Extend(size_t rounded_bytes) {
  size_t available_bytes =
      config_.max_mem - static_cast<size_t>(stats_.total_allocated_bytes);
  available_bytes = (available_bytes / kMinAllocationSize) * kMinAllocationSize;

  if (rounded_bytes > available_bytes) {
    MUSA_ARENA_RETURN_ERROR(ORT_EP_FAIL,
                            "Available memory of "
                                << available_bytes
  ...
  auto get_extend_bytes = [this, available_bytes](
                              const size_t bytes,
                              size_t& extend_bytes) -> OrtStatus* {
    extend_bytes = 0;
    if (config_.arena_extend_strategy == ArenaExtendStrategy::kNextPowerOfTwo) {
      ...
      while (bytes > curr_region_allocation_bytes_) {
        if (curr_region_allocation_bytes_ >
      ...
      extend_bytes = std::min(
          static_cast<size_t>(curr_region_allocation_bytes_), available_bytes);
      ...
      if (!increased_allocation) {
        ...
        const size_t max_extend =
            static_cast<size_t>(config_.max_power_of_two_extend_bytes);
    ...
    } else if (config_.arena_extend_strategy ==
               ArenaExtendStrategy::kSameAsRequested) {
      extend_bytes = bytes;
    } else {
      MUSA_ARENA_RETURN_ERROR(
...
#pragma warning(pop)
#endif
    if (bytes < rounded_bytes || bytes < 8 * 1024) break;
  ...
  if (mem_addr == nullptr) {
    MUSA_ARENA_RETURN_ERROR(
        ORT_EP_FAIL, "Failed to allocate memory for requested buffer of size "
                         << rounded_bytes);
  }

  MUSA_ARENA_LOG(INFO, "Extended allocation by " << bytes << " bytes.");

  // Guard against leaking mem_addr if any operation below throws (e.g. vector
  // reallocation inside AddAllocationRegion). On success we set mem_addr to
  // nullptr to dismiss the guard.
  struct AllocGuard {
    OrtAllocator* alloc;
    void*& addr;
    ~AllocGuard() {
  ...
  MUSA_ARENA_LOG(INFO,
                 "Total allocated bytes: " << stats_.total_allocated_bytes);
  MUSA_ARENA_LOG(
      INFO, "Allocated memory at "
                << mem_addr << " to "
...
void ArenaImpl::DeallocateChunk(ChunkHandle h) {
  ...
  SetChunkStream(h, nullptr);

  c->next = free_chunks_list_;
  free_chunks_list_ = h;
}

void ArenaImpl::SetChunkStream(ChunkHandle h, OrtSyncStream* stream) {
  Chunk* chunk = ChunkFromHandle(h);
  OrtSyncStream* old_stream = chunk->stream;
  const OrtSyncStreamImpl* old_impl =
      old_stream == nullptr ? nullptr : ep_api_.SyncStream_GetImpl(old_stream);
  const OrtSyncStreamImpl* new_impl =
      stream == nullptr ? nullptr : ep_api_.SyncStream_GetImpl(stream);

  ownership_index_.Assign(h, old_stream, old_impl, stream, new_impl);
  chunk->stream = stream;

  ValidateChunkStreamIndex();
}

void ArenaImpl::ValidateChunkStreamIndex() const {
#ifndef NDEBUG
  std::set<ChunkHandle> indexed_chunks;
  for (const auto& [stream_key, handles] :
       ownership_index_.stream_to_chunks()) {
    const auto* stream = static_cast<const OrtSyncStream*>(stream_key);
    assert(stream != nullptr);
    for (ChunkHandle handle : handles) {
      const Chunk* chunk = &chunks_.at(handle);
      assert(chunk->stream == stream);
      assert(indexed_chunks.insert(handle).second);
    }
  }

  for (ChunkHandle handle = 0; handle < chunks_.size(); ++handle) {
    const Chunk& chunk = chunks_[handle];
    assert((chunk.stream == nullptr) == (indexed_chunks.count(handle) == 0));
  }
#endif
}
...
void* ArenaImpl::Alloc(size_t size) {
  // A caller without an OrtSyncStream gives us no way to establish its last
  // device use. Do not put that allocation in the shared arena: concurrent
  // Runs could otherwise reuse the unowned free chunk on different streams.
  return Reserve(size);
}
...
void* ArenaImpl::Reserve(size_t size) {
  ...
  size_t allocated = 0;
  ORT_TRY { allocated = static_cast<size_t>(stats_.total_allocated_bytes); }
  ORT_CATCH(const std::exception& ex) {
    ORT_HANDLE_EXCEPTION([&]() {
      MUSA_ARENA_LOG(ERROR,
                     "Reserve: total_allocated_bytes ("
  ...
  if (allocated > config_.max_mem || size > config_.max_mem - allocated) {
    MUSA_ARENA_LOG(WARNING, "Reserve of "
                                << size << " bytes would exceed arena max_mem ("
                                << config_.max_mem << "). Returning nullptr.");
    return nullptr;
  }

  MUSA_ARENA_LOG(INFO, "Reserving memory in ArenaImpl for "
                           << allocator_name_ << " size: " << size);

  void* ptr = device_allocator_->Alloc(device_allocator_.get(), size);
  if (ptr == nullptr) {
    return nullptr;
  }
  MUSA_ARENA_ENFORCE(reserved_chunks_.find(ptr) == reserved_chunks_.end(),
                     __FUNCTION__);
...
size_t ArenaImpl::RequestedSize(const void* ptr) {
  std::lock_guard<std::mutex> lock(lock_);
  auto reserved = reserved_chunks_.find(const_cast<void*>(ptr));
  if (reserved != reserved_chunks_.end()) return reserved->second;
  ChunkHandle h = region_manager_.get_handle(ptr);
  MUSA_ARENA_ENFORCE(h != kInvalidChunkHandle, __FUNCTION__);
  Chunk* c = ChunkFromHandle(h);
...
void* ArenaImpl::AllocateRawInternal(size_t num_bytes, OrtSyncStream* stream,
  ...
  MUSA_ARENA_LOG(INFO, "Extending arena for "
                           << allocator_name_ << ". bin_num:" << bin_num
  ...
  if (status == nullptr) {
    ...
    if (chunk != nullptr) {
      ...
      status = api_.CreateStatus(ORT_EP_FAIL,
                                 ("Failed to find a free memory block despite "
                                  "calling Extend. rounded_bytes=" +
  ...
  if (dump_log_on_failure) {
    MUSA_ARENA_LOG(
        ERROR, "BFC Arena ran out of memory trying to allocate " << num_bytes);
...
OrtStatus* ArenaImpl::Shrink() {
  ...
  // Note: Reserved memory (via Reserve()) is allocated directly through the
  // device allocator and stored in reserved_chunks_, bypassing the region/chunk
  // system. Shrink() intentionally does NOT free reserved memory because it is
  // used for model initializers that must remain valid for the session
  // lifetime.
  ...
  // For each region, check if every chunk is free. If so, deallocate the
  // region.
  size_t i = 0;
  for (void* region_ptr : region_ptrs) {
    ...
    if (deallocate_region) {
      ...
      MUSA_ARENA_LOG(VERBOSE, allocator_name_ << " ArenaImpl shrunk by "
                                              << shrink_size << " bytes. "
                                              << "Total allocated is now "
                                              << stats_.total_allocated_bytes);
...
ArenaImpl::Chunk* ArenaImpl::SplitFreeChunkFromBin(
    Bin::FreeChunkSet* free_chunks, const Bin::FreeChunkSet::iterator& citer,
    size_t rounded_bytes, size_t num_bytes) {
  ...
  if (chunk->size >= rounded_bytes * 2 ||
      static_cast<int64_t>(chunk->size - rounded_bytes) >=
          config_.max_dead_bytes_per_chunk) {
  ...
  stats_.max_alloc_size = std::max<int64_t>(stats_.max_alloc_size,
...
ArenaImpl::Chunk* ArenaImpl::FindChunkPtr(BinNum bin_num, size_t rounded_bytes,
  ...
  for (; bin_num < kNumBins; bin_num++) {
    ...
    for (auto citer = b->free_chunks.begin(); citer != b->free_chunks.end();
      ...
      const ChunkHandle h = (*citer);
      Chunk* chunk = ChunkFromHandle(h);
      MUSA_ARENA_ENFORCE(!chunk->in_use(), __FUNCTION__);

      if (chunk->size >= rounded_bytes) {
        // ORT sync ids identify synchronization events, but the allocator does
        // not create an event at logical Free. Consequently an observed
        // producer/consumer wait does not prove that it covers this chunk's
        // final device access. Reuse owned chunks only on their original
        // stream; cross-stream reuse becomes possible after stream release
        // clears ownership.
        bool safe_to_use = chunk->stream == stream || !chunk->stream;

        if (safe_to_use) {
          chunk = SplitFreeChunkFromBin(&b->free_chunks, citer, rounded_bytes,
                                        num_bytes);

          if (stream) {
            SetChunkStream(h, stream);
          }
...
void ArenaImpl::SplitChunk(ChunkHandle h, size_t num_bytes) {
  ...
  Chunk* c = ChunkFromHandle(h);
  MUSA_ARENA_ENFORCE(!c->in_use() && (c->bin_num == kInvalidBinNum),
                     __FUNCTION__);

  Chunk* new_chunk = ChunkFromHandle(h_new_chunk);
  SetChunkStream(h_new_chunk, c->stream);
...
void ArenaImpl::Merge(ChunkHandle h1, ChunkHandle h2) {
  ...
  MUSA_ARENA_ENFORCE(!c1->in_use() && !c2->in_use() && c1->stream == c2->stream,
                     __FUNCTION__);

  ChunkHandle h3 = c2->next;
  c1->next = h3;
  MUSA_ARENA_ENFORCE(c2->prev == h1, __FUNCTION__);
...
void ArenaImpl::RemoveFreeChunkIterFromBin(
    Bin::FreeChunkSet* free_chunks, const Bin::FreeChunkSet::iterator& citer) {
  ChunkHandle h = *citer;
  Chunk* c = ChunkFromHandle(h);
  MUSA_ARENA_ENFORCE(!c->in_use() && (c->bin_num != kInvalidBinNum),
...
void ArenaImpl::RemoveFreeChunkFromBin(ChunkHandle h) {
  ...
  MUSA_ARENA_ENFORCE(!c->in_use() && (c->bin_num != kInvalidBinNum),
                     __FUNCTION__);
  MUSA_ARENA_ENFORCE(BinFromIndex(c->bin_num)->free_chunks.erase(h) > 0,
...
ArenaImpl::ChunkHandle ArenaImpl::Coalesce(ChunkHandle h) {
  Chunk* c = ChunkFromHandle(h);
  MUSA_ARENA_ENFORCE(!c->in_use(), __FUNCTION__);
...
std::array<ArenaImpl::BinDebugInfo, ArenaImpl::kNumBins>
ArenaImpl::GetBinDebugInfo() {
  ...
  for (const auto& region : region_manager_.regions()) {
    ...
    while (h != kInvalidChunkHandle) {
      ...
      if (c->in_use()) {
        ...
        MUSA_ARENA_ENFORCE(
            bin->free_chunks.count(h) == 1 && c->bin_num == bin_num,
            __FUNCTION__);
      }
...
void ArenaImpl::DumpMemoryLog(size_t num_bytes) {
  ...
  MUSA_ARENA_LOG(INFO, "Allocator:" << allocator_name_);
  MUSA_ARENA_LOG(INFO,
                 "Bin size: Chunks in_use/total (if not zero). Allocated bytes "
                 "in_use/total. Requested bytes.");
  ...
  for (BinNum bin_num = 0; bin_num < kNumBins; bin_num++) {
    ...
    if (bin_info.total_chunks_in_bin > 0) {
      MUSA_ARENA_LOG(INFO, b->bin_size
                               << ": Chunks " << bin_info.total_chunks_in_use
                               << "/" << bin_info.total_chunks_in_bin
                               << ". Bytes " << bin_info.total_bytes_in_use
                               << "/" << bin_info.total_bytes_in_bin << ". "
                               << "Requested "
                               << bin_info.total_requested_bytes_in_use << ".");

      waste +=
          bin_info.total_bytes_in_use - bin_info.total_requested_bytes_in_use;
    }
  }

  if (waste > 0) {
    MUSA_ARENA_LOG(INFO,
                   "Diff between in-use and requested bytes is " << waste);
  }

  Bin* b = BinForSize(num_bytes);

  MUSA_ARENA_LOG(INFO, "Bin for " << num_bytes << " bytes has max bytes of "
                                  << b->bin_size << ", Chunk State: ");

  for (ChunkHandle h : b->free_chunks) {
    Chunk* c = ChunkFromHandle(h);
    MUSA_ARENA_LOG(INFO, "  " << c->DebugString(this, true));
  }

  MUSA_ARENA_LOG(INFO, "Overall chunks summary:");
  ...
  for (const auto& region : region_manager_.regions()) {
    ...
    while (h != kInvalidChunkHandle) {
      ...
      MUSA_ARENA_LOG(INFO, (c->in_use() ? "  Chunk" : "  Free ")
                               << " at " << c->ptr << " of size " << c->size);
      h = c->next;
    }
  }

  MUSA_ARENA_LOG(INFO, "Summary of in-use chunks by size: ");
  size_t total_bytes = 0;
  for (auto& it : in_use_by_size) {
    MUSA_ARENA_LOG(INFO, "  " << it.second << " chunks of size " << it.first
                              << ". Total " << it.first * it.second);
    total_bytes += (it.first * it.second);
  }

  MUSA_ARENA_LOG(INFO, "Sum Total of in-use chunks: " << total_bytes);
  MUSA_ARENA_LOG(INFO,
                 "Stats: \n"
                     << "arena_in_use=" << stats_.bytes_in_use
                     << " total_allocated=" << stats_.total_allocated_bytes);
}

OrtStatus* ArenaImpl::ResetChunksUsingStream(
    const OrtSyncStreamImpl* stream_impl) {
  std::lock_guard<std::mutex> lock(lock_);

  const auto* stream = static_cast<const OrtSyncStream*>(
      ownership_index_.FindStream(stream_impl));
  if (stream == nullptr) {
    return nullptr;  // stream hasn't been used with this arena
  }

  const auto* chunks = ownership_index_.FindChunks(stream);
  if (chunks != nullptr) {
    const std::vector<ChunkHandle> chunk_handles(chunks->begin(),
                                                 chunks->end());
    for (ChunkHandle handle : chunk_handles) {
      Chunk* c = ChunkFromHandle(handle);
      MUSA_ARENA_ENFORCE(
          c->stream == stream,
          "Chunk " << handle << " is indexed by the wrong stream");
      SetChunkStream(handle, nullptr);
    }
  }
...
// MusaArenaAllocator factory method
/*static*/
OrtStatus* MusaArenaAllocator::Create(
    MusaAllocatorKind kind, const OrtMemoryInfo* memory_info,
    AllocatorUniquePtr raw_allocator, const OrtKeyValuePairs* options,
    size_t cache_limit_bytes, const OrtApi& api, const OrtLogger& logger,
    std::unique_ptr<MusaArenaAllocator>& out) {
  ArenaConfig config =
      options ? ArenaConfig::FromKeyValuePairs(api, *options) : ArenaConfig{};
  config.max_mem = std::min(config.max_mem, cache_limit_bytes);
  if (!config.IsValid()) {
    return api.CreateStatus(ORT_INVALID_ARGUMENT,
                            "Invalid MUSA arena allocator configuration.");
  }
  auto impl = std::make_unique<ArenaImpl>(std::move(raw_allocator), config, api,
                                          logger);
  out =
      std::make_unique<MusaArenaAllocator>(kind, memory_info, std::move(impl));
  return nullptr;
}
