...
// The factory creates the raw allocator with a deleter that knows the concrete
// type.
using AllocatorUniquePtr =
    std::unique_ptr<OrtAllocator, std::function<void(OrtAllocator*)>>;
...
// Copied from onnxruntime::OrtArenaCfg so the values and config key names
// match.
struct ArenaConfig {
  static const ArenaExtendStrategy DEFAULT_ARENA_EXTEND_STRATEGY =
  ...
  static const int64_t DEFAULT_MAX_POWER_OF_TWO_EXTEND_BYTES =
      1024 * 1024 * 1024;  // 1GB
  ...
  ArenaConfig(
      ...
      int initial_growth_chunk_size_bytes =
          DEFAULT_INITIAL_GROWTH_CHUNK_SIZE_BYTES,
      int64_t max_power_of_two_extend_bytes =
          DEFAULT_MAX_POWER_OF_TWO_EXTEND_BYTES)
      : max_mem(max_mem),
  ...
  bool IsValid() const {
    return max_mem > 0 &&
           ...
           initial_chunk_size_bytes > 0 && max_dead_bytes_per_chunk > 0 &&
  ...
  struct ConfigKeyNames {
    ...
    static constexpr const char* InitialChunkSizeBytes =
        "arena.initial_chunk_size_bytes";
    static constexpr const char* MaxDeadBytesPerChunk =
        "arena.max_dead_bytes_per_chunk";
    static constexpr const char* InitialGrowthChunkSizeBytes =
        "arena.initial_growth_chunk_size_bytes";
    static constexpr const char* MaxPowerOfTwoExtendBytes =
        "arena.max_power_of_two_extend_bytes";
    static constexpr const char* MaxMem = "arena.max_mem";
  };

  static ArenaConfig FromKeyValuePairs(const OrtApi& api,
                                       const OrtKeyValuePairs& kvps) {
    ArenaConfig config{};
    const char* value = nullptr;

    if (value = api.GetKeyValue(&kvps, ConfigKeyNames::ArenaExtendStrategy);
        value) {
      ...
      if (sval == "0") {
        ...
        config.arena_extend_strategy = static_cast<ArenaExtendStrategy>(
            -2);  // invalid — will fail IsValid()
      }
    }

    if (value = api.GetKeyValue(&kvps, ConfigKeyNames::InitialChunkSizeBytes);
        value) {
      ORT_TRY {
    ...
    if (value = api.GetKeyValue(&kvps, ConfigKeyNames::MaxDeadBytesPerChunk);
        value) {
      ORT_TRY {
        int64_t parsed = std::stoll(std::string(value));
        if (parsed <= 0 || parsed > std::numeric_limits<int>::max()) {
    ...
    if (value =
            api.GetKeyValue(&kvps, ConfigKeyNames::InitialGrowthChunkSizeBytes);
        value) {
      ORT_TRY {
        int64_t parsed = std::stoll(std::string(value));
        if (parsed <= 0 || parsed > std::numeric_limits<int>::max()) {
    ...
    if (value =
            api.GetKeyValue(&kvps, ConfigKeyNames::MaxPowerOfTwoExtendBytes);
        value) {
      ORT_TRY { config.max_power_of_two_extend_bytes = std::stoll(value); }
      ORT_CATCH(const std::exception&) {
        ORT_HANDLE_EXCEPTION([&]() {
    ...
    if (value = api.GetKeyValue(&kvps, ConfigKeyNames::MaxMem); value) {
      ...
      ORT_TRY {
        ...
        // Reject leading '-' explicitly so that e.g. "-100" doesn't become a
        // huge budget.
        if (!sval.empty() && sval[0] == '-') {
          config.max_mem = 0;  // will fail IsValid()
        } else {
          size_t parsed = static_cast<size_t>(std::stoull(sval));
          // Treat 0 as unlimited — avoids arithmetic issues and silent
          // allocation failures.
          config.max_mem =
              (parsed == 0) ? std::numeric_limits<size_t>::max() : parsed;
        }
      }
      ORT_CATCH(const std::exception&) {
        ORT_HANDLE_EXCEPTION([&]() {
...
// Macros used by ArenaImpl (adapted from plugin_ep_utils.h for the MUSA plugin
// namespace).

#define MUSA_ARENA_ENFORCE(condition, ...)                      \
  do {                                                          \
    if (!(condition)) {                                         \
      std::ostringstream oss;                                   \
      oss << "MUSA_ARENA_ENFORCE failed: " << #condition << " " \
          << __VA_ARGS__;                                       \
      throw std::runtime_error(oss.str());                      \
    }                                                           \
  } while (false)

#define MUSA_ARENA_LOG(level, ...)                                       \
  do {                                                                   \
    std::ostringstream ss;                                               \
    ss << __VA_ARGS__;                                                   \
    OrtStatus* _log_status = api_.Logger_LogMessage(                     \
        &logger_, ORT_LOGGING_LEVEL_##level, ss.str().c_str(), ORT_FILE, \
        __LINE__, __FUNCTION__);                                         \
    if (_log_status) api_.ReleaseStatus(_log_status);                    \
  } while (false)

#define MUSA_ARENA_RETURN_ERROR(code, ...)            \
  do {                                                \
    std::ostringstream ss;                            \
    ss << __VA_ARGS__;                                \
...
class ArenaImpl {
 public:
  ArenaImpl(AllocatorUniquePtr allocator, const ArenaConfig& config,
  ...
  // free. Resets growth to initial_growth_chunk_size_bytes_.
  OrtStatus* Shrink();
 ...
 private:
  ...
  struct Chunk {
    ...
    std::string DebugString(ArenaImpl* a, bool recurse) {
      std::ostringstream ss;
      ss << "  Size: " << size << " | Requested Size: " << requested_size
         << " | in_use: " << in_use();
  ...
  struct Bin {
    ...
    struct ChunkComparator {
      explicit ChunkComparator(ArenaImpl* allocator) : allocator_(allocator) {}
  ...
  class AllocationRegion {
   public:
    AllocationRegion(void* ptr, size_t memory_size, int64_t id)
        : ptr_(ptr),
          ...
          id_(id) {
      MUSA_ARENA_ENFORCE(0 == memory_size % kMinAllocationSize, __FUNCTION__);
   ...
   private:
    ...
    size_t IndexFor(const void* p) const {
      ...
      MUSA_ARENA_ENFORCE(p_int >= base_int, "AllocationRegion::IndexFor");
      MUSA_ARENA_ENFORCE(p_int < base_int + memory_size_,
                         "AllocationRegion::IndexFor");
  ...
  class RegionManager {
   public:
    ...
    void RemoveAllocationRegion(void* ptr) {
      ...
      MUSA_ARENA_ENFORCE(
          entry != regions_.end(),
          "RegionManager::RemoveAllocationRegion Could not find Region for: "
              << ptr);
  ...
  Chunk* FindChunkPtr(BinNum bin_num, size_t rounded_bytes, size_t num_bytes,
  ...
  void SetChunkStream(ChunkHandle h, OrtSyncStream* stream);
  void ValidateChunkStreamIndex() const;
  ...
  // A chunk with a non-null stream must occur in exactly that stream's set and
  // in no other set. All ownership changes go through SetChunkStream().
  ArenaOwnershipIndex ownership_index_;
...
// MusaArenaAllocator wraps ArenaImpl and presents an OrtAllocator interface.
// Inherits from BaseAllocator for uniform allocator handling.
class MusaArenaAllocator final : public BaseAllocator {
 public:
  static OrtStatus* Create(MusaAllocatorKind kind,
                           const OrtMemoryInfo* memory_info,
                           AllocatorUniquePtr raw_allocator,
                           const OrtKeyValuePairs* options,
                           size_t cache_limit_bytes, const OrtApi& api,
                           const OrtLogger& logger,
                           std::unique_ptr<MusaArenaAllocator>& out);

  MusaArenaAllocator(MusaAllocatorKind kind, const OrtMemoryInfo* memory_info,
                     std::unique_ptr<ArenaImpl> impl)
      : BaseAllocator(), memory_info_(memory_info), impl_(std::move(impl)) {
    Register(this);
    ...
    AllocOnStream =
        (kind == MusaAllocatorKind::kDevice) ? AllocOnStreamImpl : nullptr;
  }

  ~MusaArenaAllocator() override { Unregister(this); }

  static OrtStatus* ResetAllChunksUsingStream(
      const OrtSyncStreamImpl* stream_impl) {
    std::lock_guard<std::mutex> lock(RegistryMutex());
    for (auto* arena : Registry()) {
      if (OrtStatus* status = arena->ResetChunksUsingStream(stream_impl))
        return status;
    }
    return nullptr;
  }
  ...
  OrtStatus* ResetChunksUsingStream(const OrtSyncStreamImpl* stream_impl) {
    ...
    ORT_TRY { err = impl_->ResetChunksUsingStream(stream_impl); }
    ...
    ORT_CATCH(...) {
      err = Ort::GetApi().CreateStatus(
          ORT_RUNTIME_EXCEPTION,
          "MusaArenaAllocator::ResetChunksUsingStream failed with an unknown "
          "exception.");
    }
 ...
 private:
  ...
  static std::set<MusaArenaAllocator*>& Registry() {
    static auto* r = new std::set<MusaArenaAllocator*>;
    return *r;
  }
  static void Register(MusaArenaAllocator* p) {
    std::lock_guard<std::mutex> l(RegistryMutex());
    Registry().insert(p);
  }
  static void Unregister(MusaArenaAllocator* p) {
    std::lock_guard<std::mutex> l(RegistryMutex());
    Registry().erase(p);
  }

  static void* ORT_API_CALL AllocImpl(OrtAllocator* this_,
                                      size_t size) noexcept {
    ORT_TRY {
      auto& arena = *static_cast<MusaArenaAllocator*>(this_);
  ...
  static void* ORT_API_CALL ReserveImpl(OrtAllocator* this_,
                                        size_t size) noexcept {
    ORT_TRY {
      auto& arena = *static_cast<MusaArenaAllocator*>(this_);
  ...
  InfoImpl(const OrtAllocator* this_) noexcept {
    const auto& arena = *static_cast<const MusaArenaAllocator*>(this_);
  ...
  static OrtStatus* ORT_API_CALL GetStatsImpl(const OrtAllocator* this_,
    ...
    ORT_CATCH(...) {
      err = Ort::GetApi().CreateStatus(
          ORT_RUNTIME_EXCEPTION,
          "MusaArenaAllocator::GetStats failed with an unknown exception.");
    }
    return err;
  }
  ...
  static OrtStatus* ORT_API_CALL ShrinkImpl(OrtAllocator* this_) noexcept {
    ...
    ORT_CATCH(...) {
      err = Ort::GetApi().CreateStatus(
          ORT_RUNTIME_EXCEPTION,
          "MusaArenaAllocator::Shrink failed with an unknown exception.");
    }
    return err;
  }
