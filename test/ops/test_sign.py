...
def test_sign_float_like_dtypes(np_dtype, tensor_type, rtol, atol):
    x = np.array([[-2.0, 0.0, 3.0]], dtype=np_dtype)
    run_and_compare(
        "Sign",
        inputs={"X": x},
...
def test_sign_bfloat16():
    x_f32 = np.array([[-2.0, 0.0, 3.0]], dtype=np.float32)
    x = float32_to_bfloat16_bits(x_f32)
    expected = np.sign(bfloat16_bits_to_float32(x))
    ...
    np.testing.assert_allclose(bfloat16_bits_to_float32(actual), expected, rtol=0, atol=0)
